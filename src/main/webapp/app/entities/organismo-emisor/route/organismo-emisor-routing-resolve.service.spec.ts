...
import organismoEmisorResolve from './organismo-emisor-routing-resolve.service';

describe('OrganismoEmisor routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IOrganismoEmisor returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          organismoEmisorResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      // GIVEN
      vitest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IOrganismoEmisor>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
