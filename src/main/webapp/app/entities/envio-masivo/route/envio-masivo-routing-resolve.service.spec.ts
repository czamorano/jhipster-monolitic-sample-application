...
import envioMasivoResolve from './envio-masivo-routing-resolve.service';

describe('EnvioMasivo routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IEnvioMasivo returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          envioMasivoResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      // GIVEN
      vitest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IEnvioMasivo>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
