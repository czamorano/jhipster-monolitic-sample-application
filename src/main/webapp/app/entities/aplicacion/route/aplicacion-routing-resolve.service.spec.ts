...
import aplicacionResolve from './aplicacion-routing-resolve.service';
...
describe('Aplicacion routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IAplicacion returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          aplicacionResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      // GIVEN
      vitest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IAplicacion>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
